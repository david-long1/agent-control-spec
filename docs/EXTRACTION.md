...
| `generator/` | `generator/` | Ported as `agent-control-spec-generator`, the natural-language path only. The flag-driven `acs-generate init` designer stayed in AGT. Validation moved off the `opa` executable onto `validate_artifacts` and `ActivatedPolicy.from_memory`, so Rego is compiled by the engine in process. Policy targets are the AGENT-HOOKS-0.1 `$.target` envelope member rather than per-point L1 members, two of which (`$.model_request`, `$.model_response`) do not exist in an agent-hooks context. |
