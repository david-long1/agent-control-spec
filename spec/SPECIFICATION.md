...
`extends` is an ordered array of non empty parent references. A reference MAY be a string path, a string HTTPS URL, or an object with `url` and optional `integrity` or `sha256`. Existing string path entries remain valid. A file based loader resolves path entries relative to the including manifest and confines them to the directory tree rooted at the top level manifest. A file based loader resolves URL entries as HTTPS only, fetches them without ambient credentials, applies finite timeout, body size, and redirect limits, merges parents before children, and validates the merged manifest as a whole. Plain `http` and all non HTTPS URL schemes MUST fail closed. A manifest that folded in any fetched document, pinned or not, is URL sourced. A pin vouches for the fetched bytes, not for host secrets or host files. A fetched document MUST NOT name a host environment variable field (`api_key_env`, `aws_access_key_id_env`, `aws_secret_access_key_env`, `aws_session_token_env`), a host filesystem path field (`bundle`, `data`, `data_paths`, `policy_path`, `entities_path`, `schema_path`), a rego query that is not a plain rule path, or an `approval` section; a loader MUST fail closed with `runtime_error:manifest_invalid`. A fetched document MAY name a rego `bundle_url` or annotator `system_prompt_url` only when every URL hop from the root manifest to it carries a pin. A URL sourced manifest MUST NOT name a host environment variable field anywhere in the merged document. Annotation binding fields overlay the annotator declaration at dispatch, so in a URL sourced manifest a binding a fetched document supplies for an annotator it does not itself declare MUST carry only `from`, and a binding the host supplies for an annotator a fetched document declares MUST NOT carry an inline credential field (`api_key`, `headers`, `aws_access_key_id`, `aws_secret_access_key`, `aws_session_token`); a loader MUST fail closed with `runtime_error:manifest_invalid`. A declaration or binding that a host document and a fetched document both carry, identically, is the host's. URL entries with `integrity` MUST use `sha256-<base64>` over the fetched bytes. URL entries with `sha256` MUST use a 64 character hexadecimal SHA-256 digest over the fetched bytes. A mismatch MUST fail closed. `integrity` and `sha256` MUST NOT appear together.
...
A policy reads the snapshot at `context.<member>`, for example `context.tool_call.args.host` or `context.envelope.budgets.tool_call_count`, and an annotator's output at `context.annotations.<name>`. The `annotations` record is always present, and empty when no annotator ran. A snapshot member named `annotations` collides with that record and MUST fail closed with `runtime_error:policy_invocation_failed`. `$pi.policy_target`, `$pi.tool` and `$pi.intervention_point` are not part of the context.
...
- Every other JSON number becomes a Cedar `decimal`, `100.0`, `1e2` and `-0` included. A Cedar decimal carries four fractional digits, so the value is rounded to the nearest such value, ties to even: `0.123456` becomes `decimal("0.1235")`, `0.12345` becomes `decimal("0.1234")` and `0.12355` becomes `decimal("0.1236")`. A value outside the decimal range (about ±922337203685477.58) MUST fail closed. A policy compares a decimal with `decimal("...")` literals through the `lessThan`, `greaterThan` and related methods. A `Long` and a `decimal` never compare equal, and Cedar treats the two kinds of comparison differently: `<`, `<=`, `>`, `>=` and the decimal methods across the two types are evaluation errors and fail closed; `==`, `!=`, `contains`, `containsAll` and `containsAny` are value comparisons that yield `false` (`true` for `!=`) with no error. The producer of a snapshot number picks its JSON form, and `tool_call.args` is model output, so a `forbid` that tests a number for equality or set membership against a `Long` literal is silent when the number arrives as `100.0` or `1e2`. A policy set with such a gate MUST pin the attribute's type with a schema: a `Long` typed attribute rejects a request that carries a decimal, and a `decimal` typed attribute rejects the `Long` comparison when the policy set is validated. Authors SHOULD prefer ordering comparisons and the decimal methods, which fail closed across types.
- A JSON `null` record member is dropped, and a policy tests for the member with `has`. A `null` set element MUST fail closed: no guard can tell a set that lost an element from one that never had it, so a set gate such as an allowlist checked with `containsAll` would pass on the shorter set.
...
Every translation failure, and any value the Cedar runtime still rejects, MUST fail closed with `runtime_error:policy_invocation_failed`. A dispatcher MUST NOT evaluate with an empty or partial context in place of one it could not build. The dispatcher error detail names what failed: for a translation failure, the offending key as a dotted path rooted at `context`, without the value; for a schema rejection, the action only. The runtime folds every dispatcher error into the fixed verdict of section 12.3, so the detail reaches a host only through a dispatcher the host wraps.

When `schema_path` is set, the Cedar runtime checks the request against the schema. The schema MUST declare the context shape above for every action it lists. A schema whose action declares no context rejects every request, because the context is never empty. Members the snapshot may omit are declared with `"required": false`, and members that arrive as floats as `{"type": "Extension", "name": "decimal"}`. The runtime builds the context from the snapshot alone and checks it against the schema afterwards, so the schema types what the snapshot carries and converts nothing: a `decimal` typed attribute matches a JSON number, not a string, and an attribute typed as an entity never matches, because a snapshot cannot name an entity. Building the context with the schema would read a `{"type", "id"}` record as an entity reference and a string as an extension constructor argument, and `tool_call.args` is model output.

Cedar's authorization result maps to a verdict. Cedar reports the policies that contributed to its decision as a set; the runtime orders them by their position in the policy text. `Deny` maps to a `deny` verdict whose `reason` is the `@id` annotation of the first contributing `forbid`. A contributing policy without `@id`, or whose `@id` is empty or blank, yields its Cedar policy id, which is `policy<n>` for the n-th policy in the text, counted from zero. When no policy contributed, the reason is `no_matching_policy`. An evaluation error that the Cedar runtime reports for any policy, for example an unguarded read of a missing attribute, MUST fail closed with `runtime_error:policy_invocation_failed` whatever the decision; no `@id` surfaces. The dispatcher error detail for an evaluation error names the policy and the kind of error, not the Cedar runtime's message, which quotes the operands of an overflowing arithmetic operation and the argument of a failed extension call, both of which can be snapshot values. `Allow` maps to an `allow` verdict, unless a contributing `permit` carries an `advice` annotation. When several contributing permits carry advice, the most restrictive wins: `escalate` over `transform` over `warn`. Among permits whose advice has the same verdict, the first in the policy text wins. Text order gives no other precedence, so a lenient permit declared ahead of a stricter one cannot hide it. The advice of every contributing permit MUST be valid; malformed advice on one fails the request closed even when another carries valid advice. A Cedar policy author MAY produce `warn`, `escalate`, or `transform` by attaching an `advice` annotation whose JSON matches this shape.
...
The runtime treats `evidence` as opaque. It does not validate `artefact` and does not fetch any `verification_pointers`. A dispatcher that emits a non object `evidence`, an `evidence` member other than `artefact` and `verification_pointers`, or a member of the wrong JSON type (`null` counts as absent) MUST fail closed with `runtime_error:policy_output_invalid`. The error detail names the failure class and MUST NOT echo a dispatcher supplied name or value. The runtime propagates `evidence` into telemetry as defined in section 19. The URL values in `verification_pointers` MUST NOT appear in telemetry. They stay on the verdict, which the host records as AGENT-HOOKS-0.1 section 10 defines.

AGENT-HOOKS-0.1 section 5.3 caps the RFC 8785 canonical serialization of the `evidence` object at 10240 bytes, and a host's section 5 gate rejects a verdict whose evidence exceeds it. The runtime MUST hand the host evidence within that cap. Evidence over the cap degrades; the verdict does not. The runtime MUST keep `decision`, `reason`, `message`, `transform`, `result_labels`, and the dispatcher's `warnings` exactly as returned. It MUST keep `artefact` whole when `artefact` alone does not exceed the cap and drop it otherwise; it MUST NOT cut the string. It MUST then keep `verification_pointers` in RFC 8785 member order, ascending by the UTF-16 code units of the key, for as long as the result does not exceed the cap and drop the rest, so the kept pointers are a prefix of the canonical member list. That order differs from Unicode scalar value order only for keys outside the Basic Multilingual Plane. When nothing fits, `evidence` is the empty object. Evidence at or under the cap passes unchanged with no warning.

The runtime MUST mark the loss by appending one entry to `warnings[]` with `reason` `evidence_truncated` and a `message` under 256 bytes. The message states the original canonical size, the cap, whether the artefact was kept, the kept and total pointer counts, and `sha256:<lowercase-hex>` of the full canonical evidence. The size and the digest both cover the RFC 8785 canonical form of the normalized evidence object, in which an absent or `null` `artefact` and an empty `verification_pointers` map are omitted; this equals the dispatcher's own object whenever it omits `null` and empty members. A host that holds the dispatcher's full evidence can check it against that digest. The `decision` telemetry event of section 19, and the `intervention_point.transformed` event beside it, MUST carry `evidence_truncated` set to `true` in their metadata when the marker is present, because their `evidence_verification_pointer_keys` then name only the kept pointers. The runtime owns the `evidence_truncated` reason: a dispatcher `warnings` entry that carries it, returned directly or through the `warn` intent, MUST fail closed with `runtime_error:policy_output_invalid`. The degradation MUST be deterministic: the same evidence yields the same output.
...
The runtime evaluates whole snapshots and not live token streams. A host MUST assemble streamed model output before `post_model_call`, unless it adopts the incremental profile in section 18.1, and MUST assemble streamed final output before `output` in every case. Enforcement at the token level is outside this model.
...
A host that must release output before the whole response exists MAY mediate a stream as an ordered sequence of evaluated segments. Adopting this profile is optional and an implementation MAY omit it. This profile changes nothing in the runtime. Each segment is one ordinary intervention point evaluation over a whole snapshot, so statelessness and determinism are preserved. What the profile adds is the accounting that decides which already evaluated prefix the host may emit.

This profile applies only to a text stream. It does not apply to a structured streaming surface whose deltas carry fragments a policy cannot read until they are reassembled, such as a chat completion stream carrying tool call arguments split across chunks. A host guarding such a surface buffers it, because a fragment of an arguments document is not an evaluable value. Tool calls and tool results are structured values that section 18 evaluates once per concrete invocation, so this profile MUST NOT mediate them and a host MUST route them through the ordinary whole snapshot path.
...
A task clears a span by recording an outcome for it, and that task's offset advances only when the span starts at or below the offset the task has already cleared. A span starting past that offset MUST fail closed, because confirming it would release the gap between them, which nothing evaluated. Overlapping spans MUST be accepted, since a sliding or growing segmentation produces them. A span lying wholly below a task's frontier carries no new clearance, so a host MAY re-report a cleared one and an implementation MUST ignore it rather than treat it as an error. That covers only a clearing outcome. A refusal is terminal wherever it arrives, because the host is refusing text it has evaluated and no reading of that is safe to discard. A `transform` is governed by the rule below, since it asks to rewrite text rather than to report on it. A refusal that arrives after an earlier prefix was already released still withholds the remainder, so the complete refused span never reaches the caller even though a harmless prefix of it did.

A span carries a half open range of rune offsets, counted in Unicode scalar values. Byte offsets and UTF-16 code unit offsets MUST NOT be substituted for rune offsets. A span also carries a source type naming the role that produced the text. `USER_REQUEST` evaluates at `input` and `MODEL_GENERATED` evaluates at `post_model_call`. A change of role MUST start a new span.

Request text and response text occupy separate offset spaces called tracks. A host MUST NOT let progress on one track gate release on the other. Each track declares its own task set, because a task bound at one intervention point has no counterpart at the other and a shared set would stall a track on a task that can never evaluate it. An empty task set means that track is not mediated, which is the ordinary shape for a host guarding only the model stream. Payload on an unmediated track MUST fail closed, since nothing would gate it, and a session mediating neither track MUST be refused.

A task reports the offset below which it has cleared the track. The watermark for a track is the smallest offset across all of its tasks. The watermark MUST be monotonic and a host MUST report it only when it advances. A host MUST NOT release text on the strength of a faster task while a slower task is still behind it.

A session declares one safety level. Under `blocking` and `complete` a host MUST withhold a rune until the watermark for its track covers it, so no byte is emitted before the verdict covering it exists. Under `deferred` a host emits each payload as it arrives and evaluates behind the stream, accepting that content already emitted cannot be recalled. An unrecognized safety level MUST fail closed rather than resolve to the permissive level.
...
An `allow` clears, and the `warnings` it carries neither block nor alter that, because a warning is a recorded concern rather than a release decision. Their content still has to be well formed, under the rule on malformed verdicts below.

A verdict that section 5 does not admit MUST fail closed with the reserved reason `host_error:verdict_invalid` before it clears anything, because reading a decision out of a malformed verdict would release text on the strength of a substitution or an approval the verdict never carried. A `transform` whose path leaves `$target` is one such verdict, since section 14 confines a transform to the policy target.

One shape is admitted here that the section 5 wire form rejects. A `deny` carrying a `host_error:` reason is well formed, because the host that drives the profile owns that namespace and a host denying on its own interceptor failure is reporting exactly what that namespace is for. The exemption reaches no other decision, since a `host_error:` reason says that something the host relied on failed, and an `allow` or a `transform` carrying one would release or rewrite text on the strength of an evaluation that reports itself broken. It reaches no `warnings` entry under any decision either, because a warning is a recorded concern and never the host reporting its own failure. A `warnings` entry MUST carry neither reserved prefix. The `runtime_error:` namespace belongs to the runtime, and a policy that puts one on a warning would otherwise enter the record as a claim about who failed.

A `transform` permits only the substituted value, per section 17. Because the profile holds no text, the host performs the substitution and MUST NOT emit the original.

A `transform` is terminal for the session, and an implementation MUST report no watermark for a track once one is recorded. The substitution replaces the policy target with a new whole value. Its runes are not the runes the session counted, so an offset over it names a position in a sequence that no longer exists, and no task on the track evaluated it, so no clearance recorded against the original text authorizes releasing it. Both objections are fatal to an incremental release and neither is repairable by an implementation that holds no text and is never told the replacement's length. The accounting therefore stops, records that the stream was rewritten, and reports the track, the task, and the range that produced it. The track is part of the record because one task name may gate both tracks, which would otherwise leave it ambiguous which one ended. The host evaluates the replacement on the ordinary whole snapshot path of section 18, where every task sees the value actually being emitted, and starts a new session if it must keep streaming.

A `transform` MUST fail closed wherever the host can no longer alter the text it rewrites. Under `deferred` that is every transform, because the payload was emitted on arrival. Under a withholding level it is any transform recorded once a rune has been released, which includes a session resuming a partially delivered stream, whose starting offset is above zero precisely because that prefix already reached the caller.

A consequence worth stating, because it decides whether this profile suits a policy at all. Combined with the cumulative target obligation above, a host that releases as it goes has a watermark above zero after its first segment, so every later `transform` fails closed and ends the session. A policy whose ordinary answer is to rewrite, such as one redacting a value it finds mid response, therefore does not fit this profile, and a host binding one SHOULD evaluate on the whole snapshot path of section 18 instead.

Payload arrival and session settlement are distinct. A host MAY close the payload stream while outcomes are still in flight, which is what a `deferred` host needs so a late refusal can still be recorded. Payload arriving after that close MUST fail closed rather than merely being refused, because a host that ignored the refusal would settle clean over runes the accounting never counted and no task ever evaluated.
